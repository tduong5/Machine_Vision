...
def draw_bbox(image, bboxes, CLASSES=YOLO_COCO_CLASSES, show_label=True, show_confidence = True, Text_colors=(255,255,0), rectangle_colors='', tracking=False):   
    ...
    global object_name_screen
    global object_name_score

    global apple_list
    global bagel_list
    global banana_list
    global croissant_list
    global orange_list

    for i, bbox in enumerate(bboxes):
        coor = np.array(bbox[:4], dtype=np.int32)
        score = bbox[4]
        # saving this score into global variable
        object_name_score = score
        class_ind = int(bbox[5])
        # temp name
        temp_name = NUM_CLASS[class_ind]
        ...
        if show_label:
            ...
            # for CreateCSV()
            if (score > .30): # .30 to prevent accicidental detection
                if (temp_name == "Apple"):
                    apple_list.append(score)
                if (temp_name == "Bagel"):
                    bagel_list.append(score)
                if (temp_name == "Banana"):
                    banana_list.append(score)
                if (temp_name == "Croissant"):
                    croissant_list.append(score)
                if (temp_name == "Orange"):
                    orange_list.append(score)
            
            
            ## added this to store name only if score > 0.60
            if (score > .60):
                object_name_screen = NUM_CLASS[class_ind]
                purchase_Flag = True
                #print(object_name_screen)
            else:
                object_name_screen = ""
                purchase_Flag = False
                #print(object_name_screen)
...
# left this out here so Purchase_Item() would work
import sqlite3
# connect to database
connect = sqlite3.connect('user_database.db')
# allow python to execute sql command to a database
cur = connect.cursor()
# initialize global variable.. https://www.programiz.com/python-programming/global-keyword
user_name = ""
stored_ID = 0
# detect from webcam
def detect_realtime(YoloV3, output_path, input_size=416, show=False, CLASSES=YOLO_COCO_CLASSES, score_threshold=0.3, iou_threshold=0.45, rectangle_colors=''):
    ...
    # added this to detect face
    face_cascade = cv2.CascadeClassifier('./haarcascade_frontalface_default.xml') # Load the cascade for the face.
    recognizer = cv2.face.LBPHFaceRecognizer_create()
    recognizer.read('recognizer/face_training.yml')
    font = cv2.FONT_HERSHEY_SIMPLEX
    import schedule # pip install schedule
    # schedule every 3 seconds to call the function to purchase item
    schedule.every(2).seconds.do(Purchase_Item)
    # store name so Purchase_Item() gets name
    global user_name # Changing Global Variable From Inside a Function using global
    global stored_ID
    while True:
        ...
        for (x, y, w, h) in faces: # For each detected face:
            cv2.rectangle(frame, (x, y), (x+w, y+h), (0, 255, 0), 2) # Color a bounding box around the face.
            id, confidence = recognizer.predict(gray[y:y+h,x:x+w])
            face_accuracy = round(100 - confidence)
            if (confidence < 100):
                cv2.putText(frame, user_name, (x+5,y-5), font, 1, (255,255,255), 2)
                confidence = "  {0}%".format(face_accuracy)
                cv2.putText(frame, str(confidence), (x+5,y+h-5), font, 1, (255,255,0), 1)
                cur.execute("SELECT name FROM faces WHERE id = (?);", (id,))
                stored_ID = id
                result = cur.fetchall()
        ...
        # Run all jobs that are scheduled to run. Have this under 'draw_bbox' so it doesn't purchase more than once due to it calling
...
def Purchase_Item():
    # to update database with new balance
    if ((object_name_screen == 'Bagel' or object_name_screen == 'Croissant' or object_name_screen == 'Apple' or object_name_screen == 'Orange' or object_name_screen == 'Banana') and purchase_Flag):
        cur.execute("UPDATE faces SET balance = balance - 1 WHERE id = (?);", (stored_ID,))
        connect.commit()
        print("{0} purchased {1}".format(user_name, object_name_screen))   


def CreateCSV():
    import csv

    with open('plot_figures/obj_detect.csv', 'w', newline='') as file:
        col_names = ['Apple', 'Bagel', 'Banana', 'Croissant', 'Orange']
        writer = csv.DictWriter(file, fieldnames=col_names)
        writer.writeheader()
        # write under Apple col with scores that appear on screen
        for score in apple_list:
            writer.writerow({'Apple': score})

        for score in bagel_list:
            writer.writerow({'Bagel': score})

        for score in banana_list:
            writer.writerow({'Banana': score}) 
        for score in croissant_list:

            writer.writerow({'Croissant': score})
        for score in orange_list:
            writer.writerow({'Orange': score})
